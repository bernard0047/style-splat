...
class GaussianModel:
    ...
    def style_transfer_setup(self, training_args, mask3d):

        # mask3d = ~mask3d.bool().squeeze()
        # mask_xyz_values = self._xyz[~mask3d]
        # mask3d = mask3d.bool().squeeze()

        # xyz_old = self._xyz[~mask3d]
        # features_dc_old = self._features_dc[~mask3d]
        # features_rest_old = self._features_rest[~mask3d]
        # opacity_old = self._opacity[~mask3d]
        # scaling_old = self._scaling[~mask3d]
        # rotation_old = self._rotation[~mask3d]
        # objects_dc_old = self._objects_dc[~mask3d]

        # xyz_new = self._xyz[mask3d]
        # features_dc_new = self._features_dc[mask3d]
        # features_rest_new = self._features_rest[mask3d]
        # opacity_new = self._opacity[mask3d]
        # scaling_new = self._scaling[mask3d]
        # rotation_new = self._rotation[mask3d]
        # objects_dc_new = self._objects_dc[mask3d]

        # def set_requires_grad(tensor, requires_grad):
        #     """Returns a new tensor with the specified requires_grad setting."""
        #     return tensor.detach().clone().requires_grad_(requires_grad)
            
        # self._xyz = nn.Parameter(torch.cat([set_requires_grad(xyz_old, False), set_requires_grad(xyz_new, False)]))
        # self._features_dc = nn.Parameter(torch.cat([set_requires_grad(features_dc_old, False), set_requires_grad(features_dc_new, True)]))
        # self._features_rest = nn.Parameter(torch.cat([set_requires_grad(features_rest_old, False), set_requires_grad(features_rest_new, True)]))
        # self._opacity = nn.Parameter(torch.cat([set_requires_grad(opacity_old, False), set_requires_grad(opacity_new, False)]))
        # self._scaling = nn.Parameter(torch.cat([set_requires_grad(scaling_old, False), set_requires_grad(scaling_new, False)]))
        # self._rotation = nn.Parameter(torch.cat([set_requires_grad(rotation_old, False), set_requires_grad(rotation_new, False)]))
        # self._objects_dc = nn.Parameter(torch.cat([set_requires_grad(objects_dc_old, False), set_requires_grad(objects_dc_new, False)]))

        # Setup optimizer. Only the object points sh will have gradients.
        l = [
            {'params': [self._features_dc], 'lr': training_args.feature_lr, "name": "f_dc"},
            {'params': [self._features_rest], 'lr': training_args.feature_lr / 20.0, "name": "f_rest"},
            # Assuming there's a learning rate for objects_dc in training_args
        ]
